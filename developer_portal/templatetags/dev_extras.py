"""Developer-portal display helpers.

The role-aware status badge now lives in the shared matrix
(``tickets.labels`` + ``{% load ticket_labels %}``); the old
``DEV_TICKET_BADGES`` dict and ``dev_ticket_badge`` tag were removed when the
developer portal migrated onto it. This module now only re-exports the timeline
event helpers that the developer detail template still uses.
"""
